...
void *mm_malloc(size_t size)
{
    ...
    if(hook==1){
        ...
        if(head!=(char *)NULL){
            ...
            temp=temptemp;
             while(GETNEXTADDR(temp)!=(unsigned int*)NULL){
                 if(GETREALSIZE(temp)>(newsize+16))
                     break;
                 temp=(char *)GETNEXTADDR(temp);
             }
             if(GETREALSIZE(temp)>(newsize+16)){
...
void mm_free(void *ptr){
    ...
    if(!(size&(1<<0)) && !(size&(1<<1))){
        int size=GETREALSIZE(newptr)+GETREALSIZE(nextptr)+GETREALSIZE(prevptr);
        SETSIZE(prevptr,(size|0x3));
        ...
        if(nextptr!=prevcall){
            char * nextnextptr=prevptr+GETREALSIZE(prevptr);
        ...
        int size=GETREALSIZE(prevptr)+GETREALSIZE(newptr);
        SETSIZE(prevptr,(size|0x3));
        if(newptr!=prevcall){
            SETSIZE(nextptr,(GETSIZE(nextptr)&~0x1));
        ...
        int size=GETREALSIZE(newptr)+GETREALSIZE(nextptr);
        removefromlist(nextptr);
        SETSIZE(newptr,(size|0x3));
        if(newptr!=firstchunk){
            SETSIZE(prevptr,(GETSIZE(prevptr)&~0x2));
        }
        if(nextptr!=prevcall){
            char * nextnextptr=newptr+GETREALSIZE(newptr);
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if(ptr==NULL)
        ...
        memcpy(p,ptr,(GETREALSIZE(newptr)-SIZE_T_SIZE));
        mm_free(ptr);
        return (void *)(p);
    }
    return NULL;
